...
function buildScoreMatrix(homeLambda: number, awayLambda: number, maxGoals: number = 7): ScoreMatrix {
  const matrix: number[][] = [];
  ...
  for (let h = 0; h <= maxGoals; h++) {
    ...
    for (let a = 0; a <= maxGoals; a++) {
      ...
      if (h === 0 && a === 0) p = p * (1 - homeLambda * awayLambda * rho);
      else if (h === 0 && a === 1) p = p * (1 + homeLambda * rho);
      else if (h === 1 && a === 0) p = p * (1 + awayLambda * rho);
      else if (h === 1 && a === 1) p = p * (1 - rho);
...
function deriveMarketProbabilities(sm: ScoreMatrix): Record<string, number> {
  const { matrix, maxGoals } = sm;
  const cap = (v: number) => parseFloat(clamp(v, 0, 1).toFixed(4));
