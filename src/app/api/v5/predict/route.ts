...
//
// Generates a prediction for a specific match.
// Checks predictions_v2 for cached prediction (less than 6 hours old).
// If not cached, runs V5 engine and saves the result.
// ═══════════════════════════════════════════════════════════════════════
  ...
  try {
    ...
    if (!fixtureIdParam) {
      return NextResponse.json(
        { error: 'Missing fixtureId parameter' },
        { status: 400 }
      );
    }
    ...
    // ── Step 1: Check for cached prediction ────────────────────────
    try {
      ...
      if (cachedPred) {
        ...
        if (updatedAt > sixHoursAgo) {
          ...
          const cached: PredictionResult = {
            ...
            expectedGoals: safeParse(cachedPred.expected_goals_json) || { home: 0, away: 0, total: 0 },
      ...
      // predictions_v2 table may not exist yet — continue to generate
    }
